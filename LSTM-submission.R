...
#model1 = load_model_hdf5(filepath = paste0(model_dir,"model1.h5"))
#model2 = load_model_hdf5(filepath = paste0(model_dir,"model2.h5"))
#model4 = load_model_hdf5(filepath = paste0(model_dir,"model4.h5"))
#model5 = load_model_hdf5(filepath = paste0(model_dir,"model5.h5"))
...
for (i in 64:length(filelist2)) {
  train1 <- fread(file = paste0(test_dir, filelist2[i]))
  ...
  rm(list = c('context_type','hist_user_behavior_reason_end','hist_user_behavior_reason_start','hour_of_day'))
  gc()
  
  #test1_test = data.table(session_id = unique(test1[, session_id]))
  test1_test = dcast(test1, session_id ~ test_position, value.var = c('temp_1','percent_total','acoustic_vector_0','acoustic_vector_1','acoustic_vector_2','acoustic_vector_3','acoustic_vector_4','acoustic_vector_5','acoustic_vector_6','acoustic_vector_7','duration','beat_strength','tempo','release_year','us_popularity_estimate','acousticness','bounciness','danceability','dyn_range_mean','energy','flatness','instrumentalness','liveness','loudness','mechanism','organism','speechiness','time_signature','valence'))
  ...
  setorder(train1, session_id)
  gc()
  
  x_test <- train1[, .SD, .SDcols=c('skip_2_10','skip_2_9','skip_2_8','skip_2_7','skip_2_6','skip_2_5','skip_2_4','skip_2_3','skip_2_2','skip_2_1',
  ...
  for (j in c(5,6,7)) { 
    model = load_model_hdf5(filepath = paste0(model_dir,"model",j,".h5"))
    
    preds = model %>% predict_proba(x_test)
    
    setorder(test1, session_id, session_position) 
    colnames(preds) = paste0('prediction_', 1:10)
    test1_test2 = data.table(test1_test[, session_id],preds)
    setnames(test1_test2, 'V1', 'session_id')
    setnames(test1_test2, old = paste0('prediction_', 1:10), new = as.character(c(1:10)))
    
    test1_test2 = melt(test1_test2, id.vars = c('session_id'), measure.vars = as.character(c(1:10)), variable.name = 'test_position', value.name = 'prediction')
    set(test1_test2, j = 'test_position', value = as.integer(as.character((test1_test2[, test_position]))))
    
    test1 = merge(x = test1, y = test1_test2, by = c('session_id','test_position'), all.x = TRUE)
    
    setorder(test1, session_id, session_position)
    test1[, paste0('prediction',j) := prediction]
    test1[, prediction := NULL]
    rm(list = c('model','preds','test1_test2'))
    gc()
  }
  #test1[, ans := ifelse(prediction >= 0.5, 1, 0)]
  ...
  ###write.table(final_ans[,ans], paste0(sub_dir,filelist2_ans[i],1), row.names=FALSE, col.names=FALSE, quote = FALSE) 
  
  tree_scores <- data.table(read.table(file = paste0('/home/cddt/data-space/spotify-challenge/data/submissions/sub19/', filelist2_ans[i],'3raw-5.gz'), col.names = c('session_id','session_position','pred_tree5')))
  test1 = merge(x = test1, y = tree_scores, by = c('session_id','session_position'), all.x = TRUE)
  
  test1[, ans2 := ifelse((prediction5 + prediction6 + prediction7 + pred_tree5 / 5 ) / 4 >= 0.5, 1, 0)]
  final_ans2 = test1[, lapply(.SD, paste, collapse = ''), .SDcols = c('ans2'), by = 'session_id']
  write.table(final_ans2[,ans2], paste0(sub_dir,filelist2_ans[i],1), row.names=FALSE, col.names=FALSE, quote = FALSE) 
  
  test1[, ans3 := ifelse((prediction5 + prediction6 + prediction7 + pred_tree5 * 3 / 5 ) / 6 >= 0.5, 1, 0)]
  final_ans3 = test1[, lapply(.SD, paste, collapse = ''), .SDcols = c('ans3'), by = 'session_id']
  write.table(final_ans3[,ans3], paste0(sub_dir,filelist2_ans[i],2), row.names=FALSE, col.names=FALSE, quote = FALSE) 
  
  test1[, ans4 := ifelse((prediction5 + prediction6 + prediction7) / 3 >= 0.5, 1, 0)]
  final_ans4 = test1[, lapply(.SD, paste, collapse = ''), .SDcols = c('ans4'), by = 'session_id']
  write.table(final_ans4[,ans4], paste0(sub_dir,filelist2_ans[i],3), row.names=FALSE, col.names=FALSE, quote = FALSE) 
  
  test1[, ans5 := ifelse((prediction5 + pred_tree5 / 5 ) / 2 >= 0.5, 1, 0)]
  final_ans5 = test1[, lapply(.SD, paste, collapse = ''), .SDcols = c('ans5'), by = 'session_id']
  write.table(final_ans5[,ans5], paste0(sub_dir,filelist2_ans[i],4), row.names=FALSE, col.names=FALSE, quote = FALSE) 
  
  rm(list = c('tree_scores','test1','train1','test1_test','test1_test2','final_ans','final_ans2','final_ans3','final_ans4','final_ans5','xtest_data','oldcol','hist_user_behavior_reason_end','hist_user_behavior_reason_start','context_type','bst.nn','preds','x_test'))
  gc()
}
#3,4,1,2